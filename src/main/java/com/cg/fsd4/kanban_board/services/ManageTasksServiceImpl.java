...
public class ManageTasksServiceImpl implements ManageTasksService {
	...
	public List<TasksEntity> addTask(TasksEntity taskObject, int fkey) {
		TeamMemberEntity tmEntity = TMRepo.findById(fkey).get();
		TasksEntity newTask = taskRepo.save(new TasksEntity(taskObject.getTaskId(), taskObject.getTaskName(),
				taskObject.getTaskStatus(), taskObject.getWorkStatus(), tmEntity));
	...
	public TasksEntity updateTask(int fkey, TasksEntity task) throws TaskNotFoundException {
		...
		TeamMemberEntity tmEntity = TMRepo.findById(task.getTaskId()).get();
		TasksEntity newTask = taskRepo.save(new TasksEntity(task.getTaskId(), task.getTaskName(), task.getTaskStatus(),
				task.getWorkStatus(), tmEntity));
		return newTask;
	}
}